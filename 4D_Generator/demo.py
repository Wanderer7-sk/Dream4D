#!/usr/bin/env python3
"""
4D Scene Reconstruction Inference Script

This script uses the ARCroco3DStereo model to perform inference on an input sequence, generating and visualizing 4D scene reconstruction results.
Command-line arguments allow customization of model path, input sequence, image size, device, and more.

Example usage:
    python demo.py --model_path [path_to_your_model] \
        --seq_path [path_to_your_seq] --device cuda --size 512
"""
...
from add_ckpt_path import add_path_to_4d_generator
...
def prepare_input(
    ...
    from src.4d_generator.utils.image import load_images
...
def prepare_output(outputs, outdir, revisit=1, use_pose=True):
    ...
    """
    from src.4d_generator.utils.camera import pose_encoding_to_camera
    from src.4d_generator.post_process import estimate_focal_knowing_depth
    from src.4d_generator.utils.geometry import geotrf
...
def run_inference(args):
    ...
    # Add the checkpoint path (required for model imports in the 4d_generator package).
    add_path_to_4d_generator(args.model_path)

    # Import model and inference functions after adding the ckpt path.
    from src.4d_generator.inference import inference, inference_recurrent
    from src.4d_generator.model import ARCroco3DStereo
